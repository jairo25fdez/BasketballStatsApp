module.exports = function (app){
    ...
    app.delete(BASE_API_URL+"/team_stats_season", checkToken ,(request,response) =>{
    ...
    app.get(BASE_API_URL+"/team_stats_season", checkToken ,(request,response) =>{
    ...
    app.post(BASE_API_URL+"/team_stats_season", checkToken ,(request,response) =>{
        let team_stats_data = request.body;

        let team_stats = new Team_stats_season({

            team_id: team_stats_data.team_id,
            league_id: team_stats_data.league_id,
            ...
            win_percentage: team_stats_data.win_percentage,
    ...
    app.put(BASE_API_URL+"/team_stats_season", checkToken ,(request,response) =>{
    ...
    app.delete(BASE_API_URL+"/team_stats_season/:team_stats_id", checkToken ,(request,response) =>{
    ...
    app.get(BASE_API_URL+"/team_stats_season/:team_stats_id", checkToken ,(request,response) =>{
        var team_stats_id = request.params.team_stats_id;
    ...
    app.post(BASE_API_URL+"/team_stats_season/:play_id", checkToken ,(request,response) =>{
        response.sendStatus(405, "METHOD NOT ALLOWED ON A SPECIFIC CLUB.")
    });

    //PUT a specific stats in the database.
    app.put(BASE_API_URL+"/team_stats_season/:team_stats_id", checkToken ,(request,response) =>{
        ...
        Team_stats_season.findOne({_id: team_stats_id}, function (err, team_stats_data){
            if(isNull(team_stats_data)){
                ...
                team_stats_data.league_id = updatedData.league_id,
                ...
                team_stats_data.win_percentage = updatedData.win_percentage,
